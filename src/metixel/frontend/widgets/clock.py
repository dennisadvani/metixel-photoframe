...
class ClockWidget(Widget):
    ...
    def draw(self, backend: DisplayBackend) -> None:
        ...
        Kept to satisfy the widget interface; the clock's geometry and layout
        live on in :meth:`render`, which returns a description rather than
        issuing draw calls against the reduced backend.
        """

    def render(self) -> list[OverlayElement]:
        """Return the clock's elements for this frame.

        Ported from the old ``draw(backend)``: the font sizing, the horizontal
        centring heuristic and the time/date positioning are unchanged.  Only the
        mechanism differs, matching the contract the overlay layers use so there
        is one compositing model in the frontend rather than two.
        """
        ...
        # 1. Translucent background panel, painted just behind the text.
        elements.append(
        ...
        # 2. Time (and date below it).  "analog" has never been implemented and
        #    deliberately falls back to the digital rendering.
        char_width = font_size * 0.58
        text_width = char_width * len(self._current_time)
        ...
        if self._current_date:
            ...
            elements.append(
                OverlayElement.text_element(
                    self._current_date,
                    (int(date_x), int(date_y)),
                    size=date_font_size,
                    colour="#c0c0cc",
                    alpha=self._text_alpha * 0.85,
                    z=self.z_index,
                )
            )
