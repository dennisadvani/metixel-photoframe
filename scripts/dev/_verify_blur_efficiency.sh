#!/bin/bash
# Efficiency: is the blur built once per slide, and does RSS stay bounded?
PID=$(pgrep -f "mode frontend" | head -1)
echo "pid=$PID  start=$(ps -o lstart= -p $PID)"

echo
echo "=== forcing many transitions (2 s slides + 2.5 s crossfade) ==="
python3 - <<'PY'
import json, urllib.request
body = json.dumps({
    "image_duration_seconds": 2, "transition_duration_ms": 2500,
...
busy = [s for s in samples if s > 0.05]
print(f"   per-second cores: {[round(s, 2) for s in samples]}")
print(f"   peak {max(samples):.2f} cores, mean while active "
      f"{(sum(busy)/len(busy) if busy else 0):.2f} cores")
PY

echo
echo "=== RSS: must not climb across many slides ==="
for i in 1 2 3 4; do
